...
class ImageObject(db.Model):
    image_id = db.Column(db.Integer, db.ForeignKey('image.image_id'), primary_key=True)
    object_id = db.Column(db.Integer, db.ForeignKey('celestial_object.object_id'), primary_key=True)
...
class ImageGear(db.Model):
    image_id = db.Column(db.Integer, db.ForeignKey('image.image_id'), primary_key=True)
    gear_id = db.Column(db.Integer, db.ForeignKey('gear.gear_id'), primary_key=True)
...
class FrameSummary(db.Model):
    ...
    image_id = db.Column(db.Integer, db.ForeignKey('image.image_id'), unique=True)
...
class FrameSet(db.Model):
    frameset_id = db.Column(db.Integer, primary_key=True)
    image_id = db.Column(db.Integer, db.ForeignKey('image.image_id'), unique=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class RawFrame(db.Model):
    ...
    frameset_id = db.Column(db.Integer, db.ForeignKey('frame_set.frameset_id'))
